...
const Block = (props: IBlock) => {
  const { blockDimensions, blockPosition, treePositions, lightPositions } = useDecoration(props);
  ...
  return (
    <group>
      ...
      {treePositions.map(({ position, rotation, id }) => (
        <StreetTree position={position} rotation={rotation} key={id} />
      ))}
      {lightPositions.map(({ position, rotation, id }) => (
        <StreetLight position={position} rotation={rotation} key={id} />
      ))}
    </group>
  );
};
